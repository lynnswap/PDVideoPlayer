...
public extension PDVideoPlayerRepresentable {
    ...
    func tapAction(_ action: @escaping () -> Void) -> Self {
        Self(model: self.model, panGesture: self.panGesture, scrollViewConfigurator: self.scrollViewConfigurator, contextMenuProvider: self.contextMenuProvider, tapAction: action)
    }
}
#endif
